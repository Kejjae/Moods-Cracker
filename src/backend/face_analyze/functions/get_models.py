...
def load_full_EE(path):
    model = EE()
    model.load_weights(path)
    return model

def load_weights_EE_D7(path):
    model = EE_D7()
    model.load_weights(path)
    model = tf.keras.models.Model(inputs=model.input, outputs=[model.get_layer('features').output])
    return model
