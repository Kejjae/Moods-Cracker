...
class VideoCamera(object):
    def __init__(self, path_video='', conf=0.7):
        ...
        self.video = None
        self.dict_face_area = {}
        self.detector = RetinaFace(gpu_id=-1) #แก้จาก gpu_id = 0 ใช้ cpu แทน
                ...
                if self.cur_frame == j:
                    for f_id, box in enumerate(faces):
                        ...
                        if prob > self.conf:
                            ...
                            self.dict_face_area[f"{name_img}_{f_id}"] = self.channel_frame_normalization(cur_fr) #แก้บรรทัดนี้ไป
        del self.detector          
